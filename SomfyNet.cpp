...
conn_types_t SomfyNet::preferredConnType() {
...
bool SomfyNet::changeAP(const uint8_t *bssid, const int32_t channel) {
  esp_task_wdt_reset(); // Make sure we do not reboot here.
...
void SomfyNet::emitSockets(uint8_t num) {
...
void SomfyNet::setConnected(conn_types_t connType) {
  esp_task_wdt_reset();
...
bool SomfyNet::connectWired() {
  ...
  if(!this->ethStarted) {
    ...
    // core 3.x reordered the arguments of the RMII flavour of begin()
...
#else
    // no RMII EMAC on this chip, so wired ethernet is not an option
    if(true) {
#endif
#else
...
bool SomfyNet::connectWiFi(const uint8_t *bssid, const int32_t channel) {
  if(this->softAPOpened && WiFi.softAPgetStationNum() > 0) {
...
bool SomfyNet::connect(conn_types_t ctype) {
  esp_task_wdt_reset();
...
bool SomfyNet::getStrongestAP(const char *ssid, uint8_t *bssid, int32_t *channel) {
...
bool SomfyNet::connecting() {
  if(this->_connecting && millis() > this->connectStart + CONNECT_TIMEOUT) this->_connecting = false; 
  return this->_connecting; 
}
void SomfyNet::clearConnecting() { this->_connecting = false; }
void SomfyNet::networkEvent(WiFiEvent_t event) {
  switch(event) {
...
void SomfyNet::emitHeap(uint8_t num) {
  bool bEmit = false;
