...
class SomfyShade : public SomfyRemote {
  ...
  public:
    ...
    // Time from the command until the shade is halfway -- measured the same way as the
    // two above, because the command is the moment you can put a stopwatch on. A roller
    // does not travel at a constant speed: the fabric winds onto a drum whose diameter
    // grows, so it covers more distance per second when it is wound up. Zero keeps the
